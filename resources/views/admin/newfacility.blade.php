                                    ...
                                    <div class="modal-body">
                                    	<form class="" action="{{ url('admin/department_facilities') }}" method="POST">
	                                        ...
	                                        <div class="row">
                                                ...
                                                <div class="form-group {{ $errors->has('department_id') ? 'has-error': ''}} hidden">
	                                                <label class="control-label col-md-4" for="department_id">Title</label>

	                                                <div class="col-md-8">
                                                    @foreach($facilities as $i => $facility)
                                                        @if($i == 0)
                                                        <input size="16" type="text" value="{{ old('department_id', $facility->department_id) }}" id="department_id" class="form-control" name="department_id" hidden>
                                                        @endif
                                                    @endforeach 
